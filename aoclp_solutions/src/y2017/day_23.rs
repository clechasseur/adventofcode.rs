...
use crate::y2017::helpers::duet::{read_register, read_value, Registers, Value};
...
impl Instruction {
    ...
    ) -> Result<InstructionResult, aoclp::Error> {
        op_counts.inc(self);
...
impl ExperimentalCoprocessor {
    ...
    pub fn execute_next(&mut self) -> Result<InstructionResult, aoclp::Error> {
        let result = self
            .program
    ...
    pub fn execute(&mut self) -> Result<(), aoclp::Error> {
...
impl Program {
    pub fn optimize(mut self) -> Self {
        let mut optimized: Program = OPTIMIZED
            .lines()
